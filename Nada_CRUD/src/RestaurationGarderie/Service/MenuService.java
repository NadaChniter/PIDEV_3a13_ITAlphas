...
import RestaurationGarderie.IService.MenuInterface;
...
public class MenuService implements MenuInterface<Menu> {
  ...
  @Override
    public Menu getById(int id) throws SQLException {
        ...
        {
            Menu m;
            m = new Menu(rs.getInt("id"),rs.getDate("date_day"),rs.getString("plate1"),rs.getString("plate2"),rs.getString("plate3"));
            return m;
        }
        return null;
    }    
    ...
    @Override
    public boolean update(int id, Date date_day, String plate1, String plate2, String plate3) throws SQLException {
  if(exist(id))
        {
        ste=con.createStatement();
    ...
    ResultSet rs=ste.executeQuery("select * from menu");
     while (rs.next()) {                
               int id=rs.getInt(1);
               Date date_day=rs.getDate("date_day");
